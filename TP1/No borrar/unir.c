...
if (argc==1) {
  printf("\nEs aconsejable usar argumentos, ingrese la opcion -h para desplegar la ayuda\n ");
  //sto();
}
...
void sto(){
  int fdi,leer,leido;
  char nombre[30],buff[2048],buffer[40];
//  pipe(fd);
  //close(fd[0]);
  //dup2(fd[1],1)
...
//  close(fd[0]);
  ...
  if (fd==NULL) {
    ...
    fdi = open(buffer, O_RDONLY);
    leido = read(fdi, buff, 2048);
    write(1,buff,leido);
    }
//  printf("%d\n",leido );
//  sprintf(nombre, "%d",leido);
  //fdi = open(nombre, O_RDONLY);
